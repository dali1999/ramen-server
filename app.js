...
const VisitSchema = new mongoose.Schema(
    {
        ...
        visit_date: { type: Date, required: true }, // 방문 날짜
...
const RamenRestaurantSchema = new mongoose.Schema({
    ...
    bannerImageUrl: { type: String, default: "https://via.placeholder.com/150/0000FF/FFFFFF?text=NoImage" }, // 배너 이미지 URL (기본 이미지 제공)
...
const MemberSchema = new mongoose.Schema({
    ...
    imageUrl: { type: String, default: "https://via.placeholder.com/150/0000FF/FFFFFF?text=NoImage" }, // 프로필 이미지 URL (기본 이미지 제공)
});
...
const RamenRestaurant = mongoose.model("RamenRestaurant", RamenRestaurantSchema);
const Member = mongoose.model("Member", MemberSchema);
const PlannedRamenRestaurant = mongoose.model("PlannedRamenRestaurant", PlannedRamenRestaurantSchema);
...
app.post("/api/visited-ramen", async (req, res) => {
    ...
    // 필수 필드 유효성 검사
    if (!name || !visitDate) {
        return res.status(400).json({ message: "필수 필드를 모두 입력해주세요: name, location, visitDate" });
    }
    ...
    try {
        ...
        if (initialVisitMembers && initialVisitMembers.length > 0) {
            for (const member of initialVisitMembers) {
                ...
                if (!existingMember) {
                    return res.status(400).json({ message: `'${member.name}' 이름의 멤버는 존재하지 않습니다. 먼저 멤버를 추가해주세요.` });
                }
        ...
        if (restaurant) {
            ...
            const newVisit = {
                ...
                visit_date: new Date(visitDate), // 방문 날짜를 Date 객체로 변환
            ...
            res.status(200).json({ message: "라멘집 재방문 기록이 성공적으로 추가되었습니다.", restaurant });
        } else {
            ...
            res.status(201).json({ message: "새로운 라멘집이 성공적으로 추가되었습니다.", restaurant: newRamenRestaurant });
        }
    } catch (error) {
...
app.post("/api/members", async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: "멤버가 성공적으로 추가되었습니다.", member: newMember });
    } catch (error) {
        console.error("멤버 추가 오류:", error);
        if (error.code === 11000) {
            // MongoDB의 고유(unique) 인덱스 중복 오류
            return res.status(409).json({ message: `'${name}' 이름의 멤버가 이미 존재합니다.` });
        }
        res.status(500).json({ message: "서버 오류가 발생했습니다.", error: error.message });
    }
});
...
// - 아직 방문하지 않은 라멘집 정보를 추가합니다.
app.post("/api/planned-ramen", async (req, res) => {
    const { name, bannerImageUrl, location } = req.body;

    if (!name || !location) {
        return res.status(400).json({ message: "방문 예정 라멘집 이름과 위치는 필수입니다." });
    }
    ...
    try {
        ...
        await newPlannedRamen.save(); // 방문 예정 라멘집 저장
        console.log(`방문 예정 라멘집 추가: ${name}`);
        res.status(201).json({ message: "방문 예정 라멘집이 성공적으로 추가되었습니다.", plannedRamen: newPlannedRamen });
    } catch (error) {
        console.error("방문 예정 라멘집 추가 오류:", error);
        if (error.code === 11000) {
            // MongoDB의 복합 고유(unique) 인덱스 중복 오류
            return res.status(409).json({ message: `'${name}' (${location}) 은(는) 이미 방문 예정 목록에 있습니다.` });
        }
        res.status(500).json({ message: "서버 오류가 발생했습니다.", error: error.message });
    }
});
...
app.patch("/api/visited-ramen/:restaurantId/visits/:visitCount/members/:memberName/rating", async (req, res) => {
    ...
    try {
        ...
        if (memberIndex === -1) {
            ...
            return res.status(404).json({ message: `'${memberName}' 이름의 멤버를 해당 방문 기록에서 찾을 수 없습니다. 멤버를 먼저 추가해주세요.` });
        }
...
app.get("/api/visited-ramen/:id", async (req, res) => {
    try {
        const restaurant = await RamenRestaurant.findById(req.params.id); // ID로 특정 라멘집 조회
        if (!restaurant) {
            return res.status(404).json({ message: "해당 라멘집을 찾을 수 없습니다." });
        }
