...
router.post("/", authenticateToken, upload.single("bannerImage"), async (req, res, next) => {
  ...
  try {
    ...
    return res.status(400).json({ message: "유효하지 않은 멤버 데이터 형식입니다." });
  }
  ...
  try {
    ...
    if (restaurant) {
      ...
      const populatedRestaurant = await RamenRestaurant.findById(restaurant._id).populate(
      ...
      const populatedRestaurant = await RamenRestaurant.findById(newRamenRestaurant._id).populate(
        "visits.members.memberId",
        "name nickname imageUrl role"
      );
...
router.patch("/:restaurantId/visits/:visitCount/members/:memberName/rating", authenticateToken, async (req, res, next) => {
  ...
  try {
    ...
    if (!actualMember) {
      ...
      return res.status(404).json({ message: `'${memberName}' 이름의 멤버를 찾을 수 없습니다.` });
    }

    const memberInVisitIndex = targetVisit.members.findIndex((m) => m.memberId.toString() === actualMember._id.toString());
    ...
    targetVisit.visitRatingAverage =
      currentVisitMembersWithRatingCount > 0 ? currentVisitRatingsSum / currentVisitMembersWithRatingCount : 0;
    ...
    console.log(`라멘집 ${restaurant.name}의 ${visitCount}번째 방문에 '${memberName}' 멤버 별점 ${rating}, 후기 업데이트.`);
...
router.get("/:id", async (req, res, next) => {
  try {
    const restaurant = await RamenRestaurant.findById(req.params.id).populate("visits.members.memberId", "name nickname imageUrl role");
...
router.patch("/:id/images", authenticateToken, upload.array("images", 10), async (req, res, next) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (newImageUrls.length > 0) {
      ...
      if (restaurant.images.length === 0 && RamenRestaurant.schema.paths.images.defaultValue.length > 0) {
